...
__device__ inline void savedetphoton(float n_det[],uint *detectedphoton,float *ppath,MCXpos *p0,MCXdir *v,RandType t[RAND_BUF_LEN],RandType *seeddata,uint isdet,float brillouin_angle,float material){//,uint *mediaid){
    int detid;
    ...
    if(detid){
        ...
        if(baseaddr<gcfg->maxdetphoton){
            ...
            // if(SAVE_BRILLOUIN(gcfg->savedetflag))
            n_det[baseaddr++]=brillouin_angle;
//             n_det[baseaddr++]=float(*mediaid);
            n_det[baseaddr++]=material;
        }
    }
}
#endif
...
__device__ inline void savebrillouindata(float Brang,float *gBrang) {
    uint pos=atomicAdd(gbrindex,1);
    if(pos<gcfg->maxjumpdebug){
        gBrang[pos++] = Brang;
    }
}
...
__device__ inline int launchnewphoton(MCXpos *p,MCXdir *v,MCXtime *f,float3* rv,Medium *prop,uint *idx1d, OutputType *field,
                                      ...
                                      uint media[],float srcpattern[],int threadid,RandType rngseed[],RandType seeddata[],float gdebugdata[], float gBrang[],volatile int gprogress[], float brillouin_angle, float material){
    ...
    if(p->w>=0.f){
        ...
        if(gcfg->savedet){
            if((isdet&DET_MASK)==DET_MASK && *mediaid==0 && gcfg->issaveref<2) {
                savedetphoton(n_det,dpnum,ppath,p,v,photonseed,seeddata,isdet,brillouin_angle,material);
            }
...
kernel void mcx_main_loop(uint media[],OutputType field[],float genergy[],uint n_seed[],
                          ...
                          RandType *seeddata,float *gdebugdata,float *gBrang,volatile int *gprogress, float gtest_var[]) {
    ...
    bool brillouin_flag = false; // has brillouin scattering event happened for a photon yet
    float brillouin_angle = 5;
    float material = 1;
    ...
    *((float4*)(&prop))=gproperty[1];
    ...
    if(launchnewphoton<ispencil, isreflect, islabel>(&p,&v,&f,&rv,&prop,&idx1d,field,&mediaid,&w0,0,ppath,
                                                     ...
                                                     idx,(RandType*)n_seed,seeddata,gdebugdata,gBrang,gprogress,brillouin_angle,material)){
    ...
    while(f.ndone<(gcfg->threadphoton+(idx<gcfg->oddphotons))) {
        ...
        */
        ...
        if(f.pscat<=0.f) {  ///< if this photon has finished his current scattering path, calculate next scat length & angles
            ...
            // Calculated Brillouin shift
            if(!brillouin_flag){ // Check if shift has happened for this photon
                float prob=rand_uniform01(t);
                float P, s;
                x_curr = v.x; // git current trajectory
                y_curr = v.y;
                z_curr = v.z;
                if((x_curr*x_old + y_curr*y_old + z_curr*z_old) <= 0.7071) { // filtering out ~ < 3GHz shift
                    scat_ctheta = x_curr*x_old + y_curr*y_old + z_curr*z_old; // Calculate cosine of scattering angle with incident and scattering vectors
                    if(prop.g > 0) { // If forward scattering media favor backscattering for Brillouin (this is for mie scattering)
                        // 1 - cdf of Henyey-Greenstein phase function
                        P = 1.f/(1.f + prop.g);
                        s = rsqrt(1.f + prop.g*prop.g - 2.f*prop.g*scat_ctheta);
                        P = s - P;
                        P = P*(1 - prop.g*prop.g)/(2.f*prop.g);
                        P = 1.f - P;
                    }
                    else // if g < 0 then Brillouin is isotropic
                        P = rand_uniform01(t);
                    if(prob <= P) // check if uniform random distribution number is less than the probability to decide if Brillouin event should happen
                        b_prob = 1;
                    else
                        b_prob = 0;
                }
                else // Filter out values < ~ 3GHz
                    b_prob = 0;
                if(b_prob) {
                    brillouin_angle = x_curr*x_old + y_curr*y_old + z_curr*z_old; // calculate cosine of Brillouin scattering angle
                    x_old = x_curr; // Update direction vectors to be old
                    y_old = y_curr;
                    z_old = z_curr;
                    brillouin_flag = true; // set flag so Brillouin can't happen again for this photon
                    if (mediaid==0) // mediaid is an int but I need to save if as a float and the cast wasn't working
                        material = 0.f;
                    else if (mediaid==1)
                        material = 1.f;
                    else if (mediaid==2)
                        ...
                        material = 4.f;
                    if(gcfg->debuglevel & MCX_DEBUG_MOVE)
                        savebrillouindata(brillouin_angle,gBrang); // save of cosine of Brillouin angle
                }
            } // End Brillouin behavior
        ...
        *((float3*)(&p)) = (gcfg->faststep || slen==f.pscat) ? float3(p.x+len*v.x,p.y+len*v.y,p.z+len*v.z) : float3(htime.x,htime.y,htime.z);
        ...
        // where/what angle is this being launched at?
        if((mediaid==0 && (((isdet & 0xF)==0 && (!gcfg->doreflect || (gcfg->doreflect && n1==gproperty[0].w))) || (isdet==bcAbsorb || isdet==bcCyclic) )) || f.t>gcfg->twin1){
            ...
            brillouin_flag = false; // resetting flag because this is a new photon
            ...
            if(launchnewphoton<ispencil, isreflect, islabel>(&p,&v,&f,&rv,&prop,&idx1d,field,&mediaid,&w0,
                                                             ...
                                                             media,srcpattern,idx,(RandType*)n_seed,seeddata,gdebugdata,gBrang,gprogress,brillouin_angle,material))
                break;
            isdet=mediaid & DET_MASK;
            mediaid &= MED_MASK;
            continue;
        ...
        if(p.w < gcfg->minenergy){
            if(rand_do_roulette(t)*ROULETTE_SIZE<=1.f)
                ...
                brillouin_flag = false; //Relaunching again
            ...
            if(((isreflect && (isdet & 0xF)==0) || (isdet & 0x1)) && ((isdet & 0xF)==bcMirror || n1!=((gcfg->mediaformat<100)? (prop.n):(gproperty[(mediaid>0 && gcfg->mediaformat>=100)?1:mediaid].w)))){
                ...
                if(Rtotal<1.f && (((isdet & 0xF)==0 && ((gcfg->mediaformat<100) ? prop.n:gproperty[mediaid].w) >= 1.f) || isdet==bcReflect) && (isdet!=bcMirror) && rand_next_reflect(t)>Rtotal){ // do transmission
                    ...
                    if(mediaid==0){ // transmission to external boundary
                        ...
                        brillouin_flag = false; // Relaunched photon
                        if(launchnewphoton<ispencil, isreflect, islabel>(&p,&v,&f,&rv,&prop,&idx1d,field,&mediaid,&w0,
                                                                         (((idx1d==OUTSIDE_VOLUME_MAX && gcfg->bc[9+flipdir]) || (idx1d==OUTSIDE_VOLUME_MIN && gcfg->bc[6+flipdir]))? OUTSIDE_VOLUME_MIN : (mediaidold & DET_MASK)),
                                                                         ppath,n_det,detectedphoton,t,(RandType*)(sharedmem+threadIdx.x*gcfg->issaveseed*RAND_BUF_LEN*sizeof(RandType)),
                                                                         media,srcpattern,idx,(RandType*)n_seed,seeddata,gdebugdata,gBrang,gprogress,brillouin_angle,material))
...
void mcx_run_simulation(Config *cfg,GPUInfo *gpu){
    ...
    float  *gPdet,*gsrcpattern=NULL,*genergy,*greplayw=NULL,*greplaytof=NULL,*gdebugdata=NULL, *gBrang = NULL;
    float *gtest_var;
    OutputType *gfield;
    ...
    */
    ...
    unsigned int save_brillouin = 1; // For rigor could make a SAVE_BRILLOUIN mask in mcx_const.h
    unsigned int save_media = 1; // Save media for Brillouin calculation
    unsigned int hostdetreclen=partialdata+save_brillouin+save_media+SAVE_DETID(cfg->savedetflag)+3*(SAVE_PEXIT(cfg->savedetflag)+SAVE_VEXIT(cfg->savedetflag))+SAVE_W0(cfg->savedetflag); // host-side det photon data buffer length
    ...
    #pragma omp master
    {
        ...
        if(cfg->brillouin_angle==NULL)
            cfg->brillouin_angle=(float*)malloc(sizeof(float)*test_var_size); // THIS WILL NEED UPDATED WITH REAL SIZE
    ...
    test_var = (float*)calloc(test_var_size,sizeof(float)*(test_var_size));
    ...
    CUDA_ASSERT(cudaMalloc((void **) &gtest_var, sizeof(float)*test_var_size));
    ...
    *progress = 0;
    ...
    if(cfg->debuglevel & MCX_DEBUG_MOVE){
        ...
        CUDA_ASSERT(cudaMalloc((void **) &gBrang, sizeof(float)*(debuglen*cfg->maxjumpdebug)));
    }
    if(cfg->issaveseed){
    ...
    */
    ...
    for(timegate=0;timegate<totalgates;timegate+=gpu[gpuid].maxgate){
        ...
        for(iter=0;iter<ABS(cfg->respin);iter++){
            ...
            CUDA_ASSERT(cudaMemset(gPdet,0,sizeof(float)*cfg->maxdetphoton*(hostdetreclen))); // this is just setting everything in the array to 0
            CUDA_ASSERT(cudaMemset(gtest_var,0,sizeof(float)*test_var_size)); 
            if(cfg->issaveseed)
            ...
            if(cfg->debuglevel & MCX_DEBUG_MOVE){
                ...
                CUDA_ASSERT(cudaMemcpyToSymbol(gbrindex, &jumpcount, sizeof(uint), 0, cudaMemcpyHostToDevice));
            }
            ...
            switch(ispencil*100 + (isref>0)*10+(cfg->mediabyte<=4)){
                case 0:  mcx_main_loop<0,0,0> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break; // gmediaid as last input
                case 1:  mcx_main_loop<0,0,1> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
                case 10: mcx_main_loop<0,1,0> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
                case 11: mcx_main_loop<0,1,1> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
                case 100:mcx_main_loop<1,0,0> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
                case 101:mcx_main_loop<1,0,1> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
                case 110:mcx_main_loop<1,1,0> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
                case 111:mcx_main_loop<1,1,1> <<<mcgrid,mcblock,sharedbuf>>>(gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gBrang,gprogress,gtest_var);break;
            }
            #pragma omp master
            {
            ...
            if(cfg->debuglevel & MCX_DEBUG_MOVE){
                ...
                #pragma omp critical
                {
                    if(debugrec>0){
                        ...
                        cfg->exportbrillouin=(float*)realloc(cfg->exportbrillouin,(cfg->debugdatalen+debugrec)*debuglen*sizeof(float));
                        CUDA_ASSERT(cudaMemcpy(cfg->exportbrillouin+cfg->debugdatalen, gBrang,sizeof(float)*debuglen*debugrec,cudaMemcpyDeviceToHost));
            ...
            if(cfg->issavedet){
                ...
                CUDA_ASSERT(cudaMemcpy(test_var, gtest_var,sizeof(float)*test_var_size,cudaMemcpyDeviceToHost));
                ...
                if(cfg->exportdetected){
                    ...
                    {   
                        ...
                        memcpy(cfg->brillouin_angle,test_var,sizeof(float)*test_var_size);
                        if(cfg->issaveseed && cfg->seeddata)
    ...
    #pragma omp master
    {
        ...
        if(cfg->issavedet && cfg->parentid==mpStandalone && cfg->exportdetected){
            ...
            mcx_savedetphoton(cfg->exportdetected,cfg->seeddata,cfg->detectedcount,0,cfg); //NOT EDITING THIS BECAUSE NOT CONCERNED WITH FILE IO RIGHT NOW
        }
        if((cfg->debuglevel & MCX_DEBUG_MOVE) && cfg->parentid==mpStandalone && cfg->exportdebugdata){
            ...
            cfg->his.totalphoton=cfg->nphoton;
            cfg->his.detected=0;
            mcx_savedetphoton(cfg->exportdebugdata,NULL,cfg->debugdatalen,0,cfg); //NOT EDITING THIS BECAUSE NOT CONCERNED WITH FILE IO RIGHT NOW
        }
    }
    #pragma omp barrier
