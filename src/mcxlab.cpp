...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    */
    ...
    if(nlhs>=6)
        plhs[5] = mxCreateStructMatrix(ncfg,1,1,outputtag);
    if(nlhs>=7)
        plhs[6] = mxCreateStructMatrix(ncfg,1,1,outputtag);
    ...
    */
    for (jstruct = 0; jstruct < ncfg; jstruct++) {  /* how many configs */
        try{
            ...
            unsigned int save_brillouin = 1;
            hostdetreclen=partialdata+save_brillouin+1+SAVE_DETID(cfg.savedetflag)+3*(SAVE_PEXIT(cfg.savedetflag)+SAVE_VEXIT(cfg.savedetflag))+SAVE_W0(cfg.savedetflag);
            ...
            if(nlhs>=6){
                int var_size = 1;
                cfg.brillouin_angle=(float*)malloc(var_size*sizeof(float));
            }
            if(nlhs>=7){
                cfg.exportbrillouin=(float*)malloc(cfg.maxjumpdebug*sizeof(float)*MCX_DEBUG_REC_LEN);
                cfg.debuglevel |= MCX_DEBUG_MOVE;
            }
            ...
            fielddim[4]=1;
            /** if 6th output presents, output the brillouin angle */
            if(nlhs>=6){
                printf(" brillouin_angle: %f",cfg.brillouin_angle[0]);
                int var_size = 1;
                fielddim[0]=1; fielddim[1]=var_size;
                fielddim[2]=0; fielddim[3]=0;
                mxSetFieldByNumber(plhs[5],jstruct,0, mxCreateNumericArray(2,fielddim,mxSINGLE_CLASS,mxREAL));
                memcpy((float*)mxGetPr(mxGetFieldByNumber(plhs[5],jstruct,0)),cfg.brillouin_angle,fielddim[0]*fielddim[1]*sizeof(float));
                free(cfg.brillouin_angle);
                cfg.brillouin_angle=NULL;
            }
            ...
            // Export all Brillouin data (even undetected photons)
            if(nlhs>=7){
                fielddim[0]=1; fielddim[1]=cfg.debugdatalen; // his.savedphoton is for one repetition, should correct
                fielddim[2]=0; fielddim[3]=0;
                mxSetFieldByNumber(plhs[6],jstruct,0, mxCreateNumericArray(2,fielddim,mxSINGLE_CLASS,mxREAL));
                if(cfg.debuglevel & MCX_DEBUG_MOVE)
                    memcpy((float*)mxGetPr(mxGetFieldByNumber(plhs[6],jstruct,0)),cfg.exportbrillouin,fielddim[0]*fielddim[1]*sizeof(float));
                if(cfg.exportbrillouin)
                    free(cfg.exportbrillouin);
                cfg.exportbrillouin=NULL;
            }
            ...
            if(nlhs>=2){
                ...
                printf("hostdetreclen = %i",hostdetreclen); //THIS = 5 SO I WANT TO EQUAL 6 TO OUTPUT BRILLOUIN ANGLE
                // I don't think anything needs changed in this file
                fielddim[2]=0; fielddim[3]=0;
...
void mcx_validate_config(Config *cfg){
    ...
    if(cfg->wavelength==532e-9){
        printf("The wavelength is 532nm");
    }
